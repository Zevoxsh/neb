...
async function createDomainMapping(data) {
  const res = await pool.query(
    'INSERT INTO domain_mappings (hostname, proxy_id, backend_id) VALUES ($1,$2,$3) RETURNING id, hostname, proxy_id, backend_id',
    [data.hostname, data.proxyId, data.backendId]
  );
  return res.rows[0];
}
...
async function listDomainMappings() {
  ...
  return res.rows.map(r => ({ id: r.id, hostname: r.hostname, proxy_id: r.proxy_id, backend_id: r.backend_id, target_host: r.target_host, target_port: r.target_port, target_protocol: r.target_protocol }));
}
...
async function updateDomainMapping(id, data) {
  const res = await pool.query(
    'UPDATE domain_mappings SET hostname = $1, proxy_id = $2, backend_id = $3 WHERE id = $4 RETURNING id, hostname, proxy_id, backend_id',
    [data.hostname, data.proxyId, data.backendId, id]
  );
  return res.rows[0];
}
